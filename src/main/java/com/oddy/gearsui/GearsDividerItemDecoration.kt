...
class GearsDividerItemDecoration : ItemDecoration {
    ...
    private var mOnlyTopPadding = false
    private var mOnlyBottomPadding = false
    ...
    constructor(
        ...
        paddingTop: Int = 0,
        paddingBottom: Int = 0,
        onlyTopPadding: Boolean = false,
        onlyBottomPadding: Boolean = false,
        showFirstDivider: Boolean,
        showLastDivider: Boolean
    ) : this(context, resId) {
        mPaddingTop = paddingTop
        mPaddingBottom = paddingBottom
        mOnlyTopPadding = onlyTopPadding
        mOnlyBottomPadding = onlyBottomPadding
        mShowFirstDivider = showFirstDivider
        mShowLastDivider = showLastDivider
    }
    ...
    override fun getItemOffsets(
        ...
        if (mOnlyTopPadding) {
            if (position == 0) outRect.top = outRect.top + mPaddingTop
        } else {
            outRect.top = outRect.top + mPaddingTop
        }
        ...
        if (mOrientation == LinearLayoutManager.VERTICAL) {
            ...
            if (mShowLastDivider && position == state.itemCount - 1) {
                ...
                if (mOnlyBottomPadding) {
                    if (position == state.itemCount - 1) outRect.bottom =
                        outRect.bottom + mPaddingBottom
                } else {
                    outRect.bottom = outRect.bottom + mPaddingBottom
                }
            }
        } else {
